...
const PRODUCTS: Product[] = [
  ...
  // Curated enclosed picks (Amazon-searchable; safe for classrooms)
  {
    id: "flashforge-adventurer-4-pro",
    name: "FlashForge Adventurer 4 Pro",
    price: 799,
    compareAt: 899,
    desc: "Fully enclosed classroom-friendly workhorse with auto-level and fast profiles.",
    badges: ["Fully Enclosed", "Auto-Level", "HEPA Filter"],
    specs: ["220×200×250 mm", "0.4 mm nozzle", "PLA/ABS/PETG"],
    img: "/img/adventurer-4-pro.jpg",
    category: "Printers",
    rating: 4.6,
    reviews: 450,
    stock: "In stock",
  ...
  {
    id: "creality-sermoon-v1-pro",
    name: "Creality Sermoon V1 Pro",
    price: 539,
    compareAt: 599,
    desc: "Quiet, enclosed, out-of-box printing with door sensor and Wi-Fi.",
    badges: ["Fully Enclosed", "Quiet <45 dB", "Wi-Fi"],
    specs: ["175×175×165 mm", "0.4 mm nozzle", "PLA"],
    img: "/img/sermoon-v1-pro.jpg",
    category: "Printers",
    rating: 4.6,
    reviews: 320,
    stock: "In stock",
  },
  {
    ...
    price: 399,
    compareAt: 449,
    desc: "Beginner-friendly enclosed printer with assisted leveling and heated plate.",
    badges: ["Enclosed", "Assisted Level", "Heated Plate"],
    specs: ["150×150×150 mm", "0.4 mm nozzle", "PLA/PLA+"],
    img: "/img/mp-voxel.jpg",
    category: "Printers",
    rating: 4.3,
    reviews: 500,
    stock: "In stock",
  },
  {
    ...
    price: 499,
    compareAt: 549,
    desc: "Compact enclosed CoreXY with auto-level and Klipper—great for quiet classrooms.",
    badges: ["Enclosed", "Auto-Level", "CoreXY"],
    specs: ["180×180×170 mm", "0.4 mm nozzle", "PLA/TPU/PETG"],
    img: "/img/qidi-x-smart-3.jpg",
    category: "Printers",
    rating: 4.5,
    reviews: 180,
    stock: "In stock",
  ...
  {
    id: "toybox-alpha-two",
    name: "Toybox Alpha Two",
    price: 299,
    compareAt: 349,
    desc: "Tiny enclosed toy-maker for younger kids; super simple app and model library.",
    badges: ["Kid-Focused", "Enclosed", "App Library"],
    specs: ["75×80×90 mm", "0.4 mm nozzle", "PLA"],
    img: "/img/toybox-alpha-two.jpg",
    category: "Printers",
    rating: 4.4,
    reviews: 1200,
    stock: "In stock",
...
// Fallback placeholders if images aren’t uploaded yet
const PRODUCT_FALLBACKS: Record<string, string> = {
  ...
  "flashforge-adventurer-4-pro": "https://placehold.co/960x720/jpg?text=Adventurer+4+Pro",
  "creality-sermoon-v1-pro": "https://placehold.co/960x720/jpg?text=Sermoon+V1+Pro",
  "monoprice-voxel": "https://placehold.co/960x720/jpg?text=Monoprice+Voxel",
  "qidi-x-smart-3": "https://placehold.co/960x720/jpg?text=QIDI+X-Smart+3",
  "toybox-alpha-two": "https://placehold.co/960x720/jpg?text=Toybox+Alpha+Two",
};
...
/* ===================== Shop ===================== */
function Shop({ onAdd, links, products }: { onAdd: (id: string) => void; links: Record<string, string>; products: Product[] }) {
        ...
        {items.map((p) => {
          ...
          return (
            <div key={p.id} className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white backdrop-blur-sm">
              <div className="mb-3">
                <Img src={p.img} fallbackSrc={PRODUCT_FALLBACKS[p.id]} alt={p.name} fallbackLabel={p.name} className="aspect-[4/3] w-full rounded-xl border border-white/10 object-cover" />
              </div>
...
/* ===================== Bundles ===================== */
function Bundles({ links, products }: { links: Record<string, string>; products: Product[] }) {
  const amazonTag = (links["amazon_tag"] || "").trim();
...
function STLPage() {
  ...
  const reportLink = (s: STL) =>
    mailto("hello@kidprint3d.com", `Report STL: ${s.name} (${s.id})`, `Model: ${s.name}\nID: ${s.id}\nReason: (e.g., inappropriate content, unsafe design)\nDetails: `);
...
const FAQ: React.FC = () => (
  <section className="mx-auto max-w-5xl px-6 pb-12">
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 divide-y divide-white/10">
      {[
        ["Is 3D printing safe for kids?", "With supervision, enclosed printers, and PLA filament it can be classroom-friendly. We verify models and provide guidance."],
        ["What settings should we start with?", "Layer 0.2 mm, 15% infill, supports only when needed. Keep PLA for home/classroom."],
        ["Do you support school POs?", "Yes. Use Bundles to request a quote or email hello@kidprint3d.com."],
        ["How do you verify STLs?", "We check for appropriate content, risky mechanisms, and printability. You can also report any model."]
      ].map(([q, a]) => (
        <details key={q} className="p-4 group open:bg-white/7">
          <summary className="cursor-pointer list-none text-white font-semibold">{q}</summary>
    ...
    <div className="min-h-screen bg-[#0b0b0e] text-white">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0b0b0e]/85 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <button className="text-left" onClick={() => setTab("home")} aria-label="KidPrint home"><Logo /></button>
      ...
      {tab === "learn" && (
        <section className="mx-auto max-w-5xl px-6 py-12">
          ...
          <ul className="grid list-none grid-cols-1 gap-4 md:grid-cols-2">
            {[{ t: "Quickstart (10 minutes)", d: "Unbox, auto-level, load PLA, first print Benchy." }, { t: "Safety basics", d: "Supervision, enclosed printers, PLA only, ventilation, bed temps." }, { t: "Classroom setup", d: "Traffic flow, signage, roles, cleanup checklist." }, { t: "Print settings", d: "Layer 0.2mm, 15% infill, supports only when needed." }].map((x) => (
      ...
      <AnimatePresence>
        {cartOpen && (
          <motion.div className="fixed inset-0 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.aside initial={{ x: 420 }} animate={{ x: 0 }} exit={{ x: 420 }} transition={{ type: "tween", duration: 0.25 }} className="absolute right-0 top-0 h-full w-full max-w-sm bg-zinc-900 text-white shadow-xl">
              <div className="flex items-center justify-between border-b border-white/10 p-4">
                <h3 className="text-lg font-semibold">Your Cart</h3>
                <button className="rounded-xl border border-white/10 bg-white/5 p-2 hover:bg-white/10" onClick={() => setCartOpen(false)} aria-label="Close">
                  <X className="h-5 w-5" />
                </button>
              </div>
      ...
      <AnimatePresence>
        {showPrivacy && (
          <motion.div className="fixed inset-0 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} className="absolute left-1/2 top-20 w-[92vw] max-w-xl -translate-x-1/2 rounded-2xl border border-white/10 bg-zinc-900 p-5 text-sm">
